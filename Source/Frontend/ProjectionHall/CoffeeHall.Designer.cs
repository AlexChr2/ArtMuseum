namespace Ergasia3.Source.Frontend.CinemaHall
{
	partial class CoffeeHall
	{
		/// <summary>
		/// Required designer variable.
		/// </summary>
		private System.ComponentModel.IContainer components = null;

		/// <summary>
		/// Clean up any resources being used.
		/// </summary>
		/// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
		protected override void Dispose( bool disposing )
		{
			if( disposing && (components != null) )
			{
				components.Dispose();
			}
			base.Dispose( disposing );
		}

		#region Windows Form Designer generated code

		/// <summary>
		/// Required method for Designer support - do not modify
		/// the contents of this method with the code editor.
		/// </summary>
		private void InitializeComponent()
		{
			this.panel1 = new Panel();
			this.panel17 = new Panel();
			this.panel21 = new Panel();
			this.helpLbl = new LinkLabel();
			this.label1 = new Label();
			this.panel2 = new Panel();
			this.panel3 = new Panel();
			this.panel16 = new Panel();
			this.panel15 = new Panel();
			this.DrinkRbtn = new RadioButton();
			this.label27 = new Label();
			this.FoodRbtn = new RadioButton();
			this.label26 = new Label();
			this.panel6 = new Panel();
			this.panel20 = new Panel();
			this.panel19 = new Panel();
			this.panel25 = new Panel();
			this.BuyBtn = new Button();
			this.label20 = new Label();
			this.TotalPrice = new Label();
			this.label21 = new Label();
			this.panel4 = new Panel();
			this.panel5 = new Panel();
			this.panel11 = new Panel();
			this.panel13 = new Panel();
			this.Increase3Btn = new Button();
			this.Decrease3Btn = new Button();
			this.Picked3Lbl = new Label();
			this.label18 = new Label();
			this.Left3Lbl = new Label();
			this.label24 = new Label();
			this.label25 = new Label();
			this.label28 = new Label();
			this.Price3Lbl = new Label();
			this.panel24 = new Panel();
			this.Food3Pbx = new PictureBox();
			this.Food3Name = new Label();
			this.panel7 = new Panel();
			this.panel8 = new Panel();
			this.Increase2Btn = new Button();
			this.Decrease2Btn = new Button();
			this.Picked2Lbl = new Label();
			this.label6 = new Label();
			this.Left2Lbl = new Label();
			this.label11 = new Label();
			this.label13 = new Label();
			this.label14 = new Label();
			this.Price2Lbl = new Label();
			this.panel10 = new Panel();
			this.Food2Pbx = new PictureBox();
			this.Food2Name = new Label();
			this.panel12 = new Panel();
			this.panel14 = new Panel();
			this.Increase1Btn = new Button();
			this.Decrease1Btn = new Button();
			this.Picked1Lbl = new Label();
			this.label4 = new Label();
			this.Left1Lbl = new Label();
			this.label2 = new Label();
			this.label12 = new Label();
			this.label10 = new Label();
			this.Price1Lbl = new Label();
			this.panel18 = new Panel();
			this.Food1Pbx = new PictureBox();
			this.Food1Name = new Label();
			this.panel1.SuspendLayout();
			this.panel17.SuspendLayout();
			this.panel21.SuspendLayout();
			this.panel2.SuspendLayout();
			this.panel3.SuspendLayout();
			this.panel16.SuspendLayout();
			this.panel15.SuspendLayout();
			this.panel20.SuspendLayout();
			this.panel19.SuspendLayout();
			this.panel4.SuspendLayout();
			this.panel5.SuspendLayout();
			this.panel11.SuspendLayout();
			this.panel13.SuspendLayout();
			this.panel24.SuspendLayout();
			(( System.ComponentModel.ISupportInitialize )this.Food3Pbx).BeginInit();
			this.panel7.SuspendLayout();
			this.panel8.SuspendLayout();
			this.panel10.SuspendLayout();
			(( System.ComponentModel.ISupportInitialize )this.Food2Pbx).BeginInit();
			this.panel12.SuspendLayout();
			this.panel14.SuspendLayout();
			this.panel18.SuspendLayout();
			(( System.ComponentModel.ISupportInitialize )this.Food1Pbx).BeginInit();
			this.SuspendLayout();
			// 
			// panel1
			// 
			this.panel1.BackColor = Color.MediumSlateBlue;
			this.panel1.Controls.Add( this.panel17 );
			this.panel1.Controls.Add( this.panel2 );
			this.panel1.ForeColor = SystemColors.ControlText;
			this.panel1.Location = new Point( 4, 3 );
			this.panel1.Margin = new Padding( 3, 2, 3, 2 );
			this.panel1.Name = "panel1";
			this.panel1.Size = new Size( 503, 388 );
			this.panel1.TabIndex = 2;
			// 
			// panel17
			// 
			this.panel17.BackColor = Color.DarkSlateBlue;
			this.panel17.Controls.Add( this.panel21 );
			this.panel17.ForeColor = SystemColors.ControlText;
			this.panel17.Location = new Point( 3, 2 );
			this.panel17.Margin = new Padding( 3, 2, 3, 2 );
			this.panel17.Name = "panel17";
			this.panel17.Size = new Size( 497, 42 );
			this.panel17.TabIndex = 33;
			// 
			// panel21
			// 
			this.panel21.BackColor = Color.MediumSlateBlue;
			this.panel21.Controls.Add( this.helpLbl );
			this.panel21.Controls.Add( this.label1 );
			this.panel21.ForeColor = SystemColors.ControlText;
			this.panel21.Location = new Point( 3, 2 );
			this.panel21.Margin = new Padding( 3, 2, 3, 2 );
			this.panel21.Name = "panel21";
			this.panel21.Size = new Size( 492, 38 );
			this.panel21.TabIndex = 0;
			// 
			// helpLbl
			// 
			this.helpLbl.ActiveLinkColor = Color.Snow;
			this.helpLbl.AutoSize = true;
			this.helpLbl.BackColor = Color.MediumSlateBlue;
			this.helpLbl.Font = new Font( "Lucida Console", 16.2F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.helpLbl.ForeColor = SystemColors.ControlText;
			this.helpLbl.LinkColor = Color.DarkSlateBlue;
			this.helpLbl.Location = new Point( 8, 10 );
			this.helpLbl.Name = "helpLbl";
			this.helpLbl.Size = new Size( 24, 22 );
			this.helpLbl.TabIndex = 25;
			this.helpLbl.TabStop = true;
			this.helpLbl.Text = "?";
			this.helpLbl.VisitedLinkColor = Color.Snow;
			this.helpLbl.LinkClicked += this.helpLbl_LinkClicked;
			// 
			// label1
			// 
			this.label1.AutoSize = true;
			this.label1.BackColor = Color.MediumSlateBlue;
			this.label1.FlatStyle = FlatStyle.Flat;
			this.label1.Font = new Font( "Lucida Bright", 22.2F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.label1.ForeColor = Color.Snow;
			this.label1.Location = new Point( 165, 3 );
			this.label1.Name = "label1";
			this.label1.Size = new Size( 176, 34 );
			this.label1.TabIndex = 22;
			this.label1.Text = "Coffee hall";
			this.label1.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// panel2
			// 
			this.panel2.BackColor = Color.DarkSlateBlue;
			this.panel2.Controls.Add( this.panel3 );
			this.panel2.ForeColor = SystemColors.ControlText;
			this.panel2.Location = new Point( 4, 46 );
			this.panel2.Margin = new Padding( 3, 2, 3, 2 );
			this.panel2.Name = "panel2";
			this.panel2.Size = new Size( 496, 338 );
			this.panel2.TabIndex = 46;
			// 
			// panel3
			// 
			this.panel3.BackColor = Color.MediumSlateBlue;
			this.panel3.Controls.Add( this.panel16 );
			this.panel3.Controls.Add( this.panel20 );
			this.panel3.Controls.Add( this.panel4 );
			this.panel3.ForeColor = SystemColors.ControlText;
			this.panel3.Location = new Point( 3, 3 );
			this.panel3.Margin = new Padding( 3, 2, 3, 2 );
			this.panel3.Name = "panel3";
			this.panel3.Size = new Size( 491, 332 );
			this.panel3.TabIndex = 0;
			// 
			// panel16
			// 
			this.panel16.BackColor = Color.DarkSlateBlue;
			this.panel16.Controls.Add( this.panel15 );
			this.panel16.ForeColor = SystemColors.ControlText;
			this.panel16.Location = new Point( 3, 2 );
			this.panel16.Margin = new Padding( 3, 2, 3, 2 );
			this.panel16.Name = "panel16";
			this.panel16.Size = new Size( 98, 327 );
			this.panel16.TabIndex = 56;
			// 
			// panel15
			// 
			this.panel15.BackColor = Color.MediumSlateBlue;
			this.panel15.Controls.Add( this.DrinkRbtn );
			this.panel15.Controls.Add( this.label27 );
			this.panel15.Controls.Add( this.FoodRbtn );
			this.panel15.Controls.Add( this.label26 );
			this.panel15.Controls.Add( this.panel6 );
			this.panel15.ForeColor = SystemColors.ControlText;
			this.panel15.Location = new Point( 3, 2 );
			this.panel15.Margin = new Padding( 3, 2, 3, 2 );
			this.panel15.Name = "panel15";
			this.panel15.Size = new Size( 93, 322 );
			this.panel15.TabIndex = 49;
			// 
			// DrinkRbtn
			// 
			this.DrinkRbtn.AutoSize = true;
			this.DrinkRbtn.Location = new Point( 39, 248 );
			this.DrinkRbtn.Margin = new Padding( 3, 2, 3, 2 );
			this.DrinkRbtn.Name = "DrinkRbtn";
			this.DrinkRbtn.Size = new Size( 14, 13 );
			this.DrinkRbtn.TabIndex = 80;
			this.DrinkRbtn.UseVisualStyleBackColor = true;
			this.DrinkRbtn.CheckedChanged += this.DrinkRbtn_CheckedChanged;
			// 
			// label27
			// 
			this.label27.AutoSize = true;
			this.label27.BackColor = Color.MediumSlateBlue;
			this.label27.FlatStyle = FlatStyle.Flat;
			this.label27.Font = new Font( "Lucida Bright", 13.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.label27.ForeColor = Color.DarkSlateBlue;
			this.label27.Location = new Point( 9, 223 );
			this.label27.Name = "label27";
			this.label27.Size = new Size( 74, 22 );
			this.label27.TabIndex = 79;
			this.label27.Text = "Drinks";
			this.label27.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// FoodRbtn
			// 
			this.FoodRbtn.AutoSize = true;
			this.FoodRbtn.Location = new Point( 40, 87 );
			this.FoodRbtn.Margin = new Padding( 3, 2, 3, 2 );
			this.FoodRbtn.Name = "FoodRbtn";
			this.FoodRbtn.Size = new Size( 14, 13 );
			this.FoodRbtn.TabIndex = 78;
			this.FoodRbtn.UseVisualStyleBackColor = true;
			this.FoodRbtn.CheckedChanged += this.FoodRbtn_CheckedChanged;
			// 
			// label26
			// 
			this.label26.AutoSize = true;
			this.label26.BackColor = Color.MediumSlateBlue;
			this.label26.FlatStyle = FlatStyle.Flat;
			this.label26.Font = new Font( "Lucida Bright", 13.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.label26.ForeColor = Color.DarkSlateBlue;
			this.label26.Location = new Point( 12, 62 );
			this.label26.Name = "label26";
			this.label26.Size = new Size( 71, 22 );
			this.label26.TabIndex = 77;
			this.label26.Text = "Foods";
			this.label26.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// panel6
			// 
			this.panel6.BackColor = Color.DarkSlateBlue;
			this.panel6.ForeColor = SystemColors.ControlText;
			this.panel6.Location = new Point( 10, 159 );
			this.panel6.Margin = new Padding( 3, 2, 3, 2 );
			this.panel6.Name = "panel6";
			this.panel6.Size = new Size( 74, 4 );
			this.panel6.TabIndex = 57;
			// 
			// panel20
			// 
			this.panel20.BackColor = Color.DarkSlateBlue;
			this.panel20.Controls.Add( this.panel19 );
			this.panel20.ForeColor = SystemColors.ControlText;
			this.panel20.Location = new Point( 103, 287 );
			this.panel20.Margin = new Padding( 3, 2, 3, 2 );
			this.panel20.Name = "panel20";
			this.panel20.Size = new Size( 385, 42 );
			this.panel20.TabIndex = 3;
			// 
			// panel19
			// 
			this.panel19.BackColor = Color.MediumSlateBlue;
			this.panel19.Controls.Add( this.panel25 );
			this.panel19.Controls.Add( this.BuyBtn );
			this.panel19.Controls.Add( this.label20 );
			this.panel19.Controls.Add( this.TotalPrice );
			this.panel19.Controls.Add( this.label21 );
			this.panel19.ForeColor = SystemColors.ControlText;
			this.panel19.Location = new Point( 3, 2 );
			this.panel19.Margin = new Padding( 3, 2, 3, 2 );
			this.panel19.Name = "panel19";
			this.panel19.Size = new Size( 380, 38 );
			this.panel19.TabIndex = 2;
			// 
			// panel25
			// 
			this.panel25.BackColor = Color.DarkSlateBlue;
			this.panel25.ForeColor = SystemColors.ControlText;
			this.panel25.Location = new Point( 228, 5 );
			this.panel25.Margin = new Padding( 3, 2, 3, 2 );
			this.panel25.Name = "panel25";
			this.panel25.Size = new Size( 4, 28 );
			this.panel25.TabIndex = 77;
			// 
			// BuyBtn
			// 
			this.BuyBtn.BackColor = Color.DarkSlateBlue;
			this.BuyBtn.Font = new Font( "Lucida Calligraphy", 9F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.BuyBtn.ForeColor = Color.Snow;
			this.BuyBtn.Location = new Point( 250, 5 );
			this.BuyBtn.Margin = new Padding( 3, 2, 3, 2 );
			this.BuyBtn.Name = "BuyBtn";
			this.BuyBtn.Size = new Size( 114, 28 );
			this.BuyBtn.TabIndex = 86;
			this.BuyBtn.Text = "Buy";
			this.BuyBtn.UseVisualStyleBackColor = false;
			this.BuyBtn.Click += this.BuyBtn_Click;
			// 
			// label20
			// 
			this.label20.AutoSize = true;
			this.label20.BackColor = Color.MediumSlateBlue;
			this.label20.FlatStyle = FlatStyle.Flat;
			this.label20.Font = new Font( "Lucida Bright", 13.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.label20.ForeColor = Color.Snow;
			this.label20.Location = new Point( 13, 10 );
			this.label20.Name = "label20";
			this.label20.Size = new Size( 121, 22 );
			this.label20.TabIndex = 76;
			this.label20.Text = "Total price:";
			this.label20.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// TotalPrice
			// 
			this.TotalPrice.AutoSize = true;
			this.TotalPrice.BackColor = Color.MediumSlateBlue;
			this.TotalPrice.FlatStyle = FlatStyle.Flat;
			this.TotalPrice.Font = new Font( "Lucida Bright", 13.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.TotalPrice.ForeColor = Color.DarkSlateBlue;
			this.TotalPrice.Location = new Point( 136, 10 );
			this.TotalPrice.Name = "TotalPrice";
			this.TotalPrice.Size = new Size( 63, 22 );
			this.TotalPrice.TabIndex = 74;
			this.TotalPrice.Text = "12.30";
			this.TotalPrice.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// label21
			// 
			this.label21.AutoSize = true;
			this.label21.BackColor = Color.MediumSlateBlue;
			this.label21.FlatStyle = FlatStyle.Flat;
			this.label21.Font = new Font( "Lucida Bright", 13.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.label21.ForeColor = Color.Snow;
			this.label21.Location = new Point( 197, 10 );
			this.label21.Name = "label21";
			this.label21.Size = new Size( 22, 22 );
			this.label21.TabIndex = 75;
			this.label21.Text = "$";
			this.label21.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// panel4
			// 
			this.panel4.BackColor = Color.DarkSlateBlue;
			this.panel4.Controls.Add( this.panel5 );
			this.panel4.ForeColor = SystemColors.ControlText;
			this.panel4.Location = new Point( 103, 2 );
			this.panel4.Margin = new Padding( 3, 2, 3, 2 );
			this.panel4.Name = "panel4";
			this.panel4.Size = new Size( 385, 284 );
			this.panel4.TabIndex = 1;
			// 
			// panel5
			// 
			this.panel5.BackColor = Color.MediumSlateBlue;
			this.panel5.Controls.Add( this.panel11 );
			this.panel5.Controls.Add( this.panel7 );
			this.panel5.Controls.Add( this.panel12 );
			this.panel5.ForeColor = SystemColors.ControlText;
			this.panel5.Location = new Point( 3, 2 );
			this.panel5.Margin = new Padding( 3, 2, 3, 2 );
			this.panel5.Name = "panel5";
			this.panel5.Size = new Size( 380, 278 );
			this.panel5.TabIndex = 0;
			// 
			// panel11
			// 
			this.panel11.BackColor = Color.DarkSlateBlue;
			this.panel11.Controls.Add( this.panel13 );
			this.panel11.ForeColor = SystemColors.ControlText;
			this.panel11.Location = new Point( 3, 185 );
			this.panel11.Margin = new Padding( 3, 2, 3, 2 );
			this.panel11.Name = "panel11";
			this.panel11.Size = new Size( 374, 91 );
			this.panel11.TabIndex = 68;
			// 
			// panel13
			// 
			this.panel13.BackColor = Color.MediumSlateBlue;
			this.panel13.Controls.Add( this.Increase3Btn );
			this.panel13.Controls.Add( this.Decrease3Btn );
			this.panel13.Controls.Add( this.Picked3Lbl );
			this.panel13.Controls.Add( this.label18 );
			this.panel13.Controls.Add( this.Left3Lbl );
			this.panel13.Controls.Add( this.label24 );
			this.panel13.Controls.Add( this.label25 );
			this.panel13.Controls.Add( this.label28 );
			this.panel13.Controls.Add( this.Price3Lbl );
			this.panel13.Controls.Add( this.panel24 );
			this.panel13.Controls.Add( this.Food3Name );
			this.panel13.ForeColor = SystemColors.ControlText;
			this.panel13.Location = new Point( 2, 2 );
			this.panel13.Margin = new Padding( 3, 2, 3, 2 );
			this.panel13.Name = "panel13";
			this.panel13.Size = new Size( 370, 86 );
			this.panel13.TabIndex = 59;
			// 
			// Increase3Btn
			// 
			this.Increase3Btn.BackColor = Color.DarkSlateBlue;
			this.Increase3Btn.Font = new Font( "Lucida Bright", 13.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Increase3Btn.ForeColor = Color.Snow;
			this.Increase3Btn.Location = new Point( 320, 53 );
			this.Increase3Btn.Margin = new Padding( 3, 2, 3, 2 );
			this.Increase3Btn.Name = "Increase3Btn";
			this.Increase3Btn.Padding = new Padding( 3, 0, 0, 0 );
			this.Increase3Btn.Size = new Size( 39, 28 );
			this.Increase3Btn.TabIndex = 85;
			this.Increase3Btn.Text = "+";
			this.Increase3Btn.UseVisualStyleBackColor = false;
			this.Increase3Btn.Click += this.ChangeItemQuantity_Click;
			// 
			// Decrease3Btn
			// 
			this.Decrease3Btn.BackColor = Color.DarkSlateBlue;
			this.Decrease3Btn.Font = new Font( "Lucida Bright", 13.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Decrease3Btn.ForeColor = Color.Snow;
			this.Decrease3Btn.Location = new Point( 246, 53 );
			this.Decrease3Btn.Margin = new Padding( 3, 2, 3, 2 );
			this.Decrease3Btn.Name = "Decrease3Btn";
			this.Decrease3Btn.Padding = new Padding( 3, 0, 0, 0 );
			this.Decrease3Btn.Size = new Size( 39, 28 );
			this.Decrease3Btn.TabIndex = 84;
			this.Decrease3Btn.Text = "-";
			this.Decrease3Btn.UseVisualStyleBackColor = false;
			this.Decrease3Btn.Click += this.ChangeItemQuantity_Click;
			// 
			// Picked3Lbl
			// 
			this.Picked3Lbl.AutoSize = true;
			this.Picked3Lbl.BackColor = Color.MediumSlateBlue;
			this.Picked3Lbl.FlatStyle = FlatStyle.Flat;
			this.Picked3Lbl.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Picked3Lbl.ForeColor = Color.Snow;
			this.Picked3Lbl.Location = new Point( 186, 34 );
			this.Picked3Lbl.Name = "Picked3Lbl";
			this.Picked3Lbl.Size = new Size( 17, 17 );
			this.Picked3Lbl.TabIndex = 79;
			this.Picked3Lbl.Text = "3";
			this.Picked3Lbl.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// label18
			// 
			this.label18.AutoSize = true;
			this.label18.BackColor = Color.MediumSlateBlue;
			this.label18.FlatStyle = FlatStyle.Flat;
			this.label18.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.label18.ForeColor = Color.DarkSlateBlue;
			this.label18.Location = new Point( 112, 5 );
			this.label18.Name = "label18";
			this.label18.Size = new Size( 41, 17 );
			this.label18.TabIndex = 78;
			this.label18.Text = "Left:";
			this.label18.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// Left3Lbl
			// 
			this.Left3Lbl.AutoSize = true;
			this.Left3Lbl.BackColor = Color.MediumSlateBlue;
			this.Left3Lbl.FlatStyle = FlatStyle.Flat;
			this.Left3Lbl.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Left3Lbl.ForeColor = Color.Snow;
			this.Left3Lbl.Location = new Point( 162, 5 );
			this.Left3Lbl.Name = "Left3Lbl";
			this.Left3Lbl.Size = new Size( 17, 17 );
			this.Left3Lbl.TabIndex = 77;
			this.Left3Lbl.Text = "9";
			this.Left3Lbl.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// label24
			// 
			this.label24.AutoSize = true;
			this.label24.BackColor = Color.MediumSlateBlue;
			this.label24.FlatStyle = FlatStyle.Flat;
			this.label24.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.label24.ForeColor = Color.DarkSlateBlue;
			this.label24.Location = new Point( 112, 34 );
			this.label24.Name = "label24";
			this.label24.Size = new Size( 61, 17 );
			this.label24.TabIndex = 73;
			this.label24.Text = "Picked:";
			this.label24.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// label25
			// 
			this.label25.AutoSize = true;
			this.label25.BackColor = Color.MediumSlateBlue;
			this.label25.FlatStyle = FlatStyle.Flat;
			this.label25.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.label25.ForeColor = Color.DarkSlateBlue;
			this.label25.Location = new Point( 112, 65 );
			this.label25.Name = "label25";
			this.label25.Size = new Size( 49, 17 );
			this.label25.TabIndex = 70;
			this.label25.Text = "Price:";
			this.label25.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// label28
			// 
			this.label28.AutoSize = true;
			this.label28.BackColor = Color.MediumSlateBlue;
			this.label28.FlatStyle = FlatStyle.Flat;
			this.label28.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.label28.ForeColor = Color.DarkSlateBlue;
			this.label28.Location = new Point( 211, 65 );
			this.label28.Name = "label28";
			this.label28.Size = new Size( 17, 17 );
			this.label28.TabIndex = 60;
			this.label28.Text = "$";
			this.label28.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// Price3Lbl
			// 
			this.Price3Lbl.AutoSize = true;
			this.Price3Lbl.BackColor = Color.MediumSlateBlue;
			this.Price3Lbl.FlatStyle = FlatStyle.Flat;
			this.Price3Lbl.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Price3Lbl.ForeColor = Color.Snow;
			this.Price3Lbl.Location = new Point( 162, 65 );
			this.Price3Lbl.Name = "Price3Lbl";
			this.Price3Lbl.Size = new Size( 48, 17 );
			this.Price3Lbl.TabIndex = 59;
			this.Price3Lbl.Text = "12.30";
			this.Price3Lbl.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// panel24
			// 
			this.panel24.BackColor = Color.DarkSlateBlue;
			this.panel24.Controls.Add( this.Food3Pbx );
			this.panel24.ForeColor = SystemColors.ControlText;
			this.panel24.Location = new Point( 3, 3 );
			this.panel24.Margin = new Padding( 3, 2, 3, 2 );
			this.panel24.Name = "panel24";
			this.panel24.Size = new Size( 100, 80 );
			this.panel24.TabIndex = 53;
			// 
			// Food3Pbx
			// 
			this.Food3Pbx.BackColor = Color.MediumSlateBlue;
			this.Food3Pbx.ForeColor = SystemColors.ControlText;
			this.Food3Pbx.Location = new Point( 3, 2 );
			this.Food3Pbx.Margin = new Padding( 3, 2, 3, 2 );
			this.Food3Pbx.Name = "Food3Pbx";
			this.Food3Pbx.Size = new Size( 94, 76 );
			this.Food3Pbx.SizeMode = PictureBoxSizeMode.StretchImage;
			this.Food3Pbx.TabIndex = 55;
			this.Food3Pbx.TabStop = false;
			// 
			// Food3Name
			// 
			this.Food3Name.AutoSize = true;
			this.Food3Name.BackColor = Color.MediumSlateBlue;
			this.Food3Name.FlatStyle = FlatStyle.Flat;
			this.Food3Name.Font = new Font( "Lucida Bright", 13.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Food3Name.ForeColor = Color.Snow;
			this.Food3Name.Location = new Point( 242, 5 );
			this.Food3Name.Name = "Food3Name";
			this.Food3Name.Size = new Size( 115, 22 );
			this.Food3Name.TabIndex = 54;
			this.Food3Name.Text = "FoodName";
			this.Food3Name.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// panel7
			// 
			this.panel7.BackColor = Color.DarkSlateBlue;
			this.panel7.Controls.Add( this.panel8 );
			this.panel7.ForeColor = SystemColors.ControlText;
			this.panel7.Location = new Point( 3, 94 );
			this.panel7.Margin = new Padding( 3, 2, 3, 2 );
			this.panel7.Name = "panel7";
			this.panel7.Size = new Size( 374, 91 );
			this.panel7.TabIndex = 68;
			// 
			// panel8
			// 
			this.panel8.BackColor = Color.MediumSlateBlue;
			this.panel8.Controls.Add( this.Increase2Btn );
			this.panel8.Controls.Add( this.Decrease2Btn );
			this.panel8.Controls.Add( this.Picked2Lbl );
			this.panel8.Controls.Add( this.label6 );
			this.panel8.Controls.Add( this.Left2Lbl );
			this.panel8.Controls.Add( this.label11 );
			this.panel8.Controls.Add( this.label13 );
			this.panel8.Controls.Add( this.label14 );
			this.panel8.Controls.Add( this.Price2Lbl );
			this.panel8.Controls.Add( this.panel10 );
			this.panel8.Controls.Add( this.Food2Name );
			this.panel8.ForeColor = SystemColors.ControlText;
			this.panel8.Location = new Point( 2, 2 );
			this.panel8.Margin = new Padding( 3, 2, 3, 2 );
			this.panel8.Name = "panel8";
			this.panel8.Size = new Size( 370, 86 );
			this.panel8.TabIndex = 59;
			// 
			// Increase2Btn
			// 
			this.Increase2Btn.BackColor = Color.DarkSlateBlue;
			this.Increase2Btn.Font = new Font( "Lucida Bright", 13.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Increase2Btn.ForeColor = Color.Snow;
			this.Increase2Btn.Location = new Point( 320, 52 );
			this.Increase2Btn.Margin = new Padding( 3, 2, 3, 2 );
			this.Increase2Btn.Name = "Increase2Btn";
			this.Increase2Btn.Padding = new Padding( 3, 0, 0, 0 );
			this.Increase2Btn.Size = new Size( 39, 28 );
			this.Increase2Btn.TabIndex = 83;
			this.Increase2Btn.Text = "+";
			this.Increase2Btn.UseVisualStyleBackColor = false;
			this.Increase2Btn.Click += this.ChangeItemQuantity_Click;
			// 
			// Decrease2Btn
			// 
			this.Decrease2Btn.BackColor = Color.DarkSlateBlue;
			this.Decrease2Btn.Font = new Font( "Lucida Bright", 13.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Decrease2Btn.ForeColor = Color.Snow;
			this.Decrease2Btn.Location = new Point( 246, 52 );
			this.Decrease2Btn.Margin = new Padding( 3, 2, 3, 2 );
			this.Decrease2Btn.Name = "Decrease2Btn";
			this.Decrease2Btn.Padding = new Padding( 3, 0, 0, 0 );
			this.Decrease2Btn.Size = new Size( 39, 28 );
			this.Decrease2Btn.TabIndex = 82;
			this.Decrease2Btn.Text = "-";
			this.Decrease2Btn.UseVisualStyleBackColor = false;
			this.Decrease2Btn.Click += this.ChangeItemQuantity_Click;
			// 
			// Picked2Lbl
			// 
			this.Picked2Lbl.AutoSize = true;
			this.Picked2Lbl.BackColor = Color.MediumSlateBlue;
			this.Picked2Lbl.FlatStyle = FlatStyle.Flat;
			this.Picked2Lbl.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Picked2Lbl.ForeColor = Color.Snow;
			this.Picked2Lbl.Location = new Point( 186, 34 );
			this.Picked2Lbl.Name = "Picked2Lbl";
			this.Picked2Lbl.Size = new Size( 17, 17 );
			this.Picked2Lbl.TabIndex = 79;
			this.Picked2Lbl.Text = "3";
			this.Picked2Lbl.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// label6
			// 
			this.label6.AutoSize = true;
			this.label6.BackColor = Color.MediumSlateBlue;
			this.label6.FlatStyle = FlatStyle.Flat;
			this.label6.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.label6.ForeColor = Color.DarkSlateBlue;
			this.label6.Location = new Point( 112, 5 );
			this.label6.Name = "label6";
			this.label6.Size = new Size( 41, 17 );
			this.label6.TabIndex = 78;
			this.label6.Text = "Left:";
			this.label6.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// Left2Lbl
			// 
			this.Left2Lbl.AutoSize = true;
			this.Left2Lbl.BackColor = Color.MediumSlateBlue;
			this.Left2Lbl.FlatStyle = FlatStyle.Flat;
			this.Left2Lbl.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Left2Lbl.ForeColor = Color.Snow;
			this.Left2Lbl.Location = new Point( 162, 5 );
			this.Left2Lbl.Name = "Left2Lbl";
			this.Left2Lbl.Size = new Size( 17, 17 );
			this.Left2Lbl.TabIndex = 77;
			this.Left2Lbl.Text = "9";
			this.Left2Lbl.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// label11
			// 
			this.label11.AutoSize = true;
			this.label11.BackColor = Color.MediumSlateBlue;
			this.label11.FlatStyle = FlatStyle.Flat;
			this.label11.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.label11.ForeColor = Color.DarkSlateBlue;
			this.label11.Location = new Point( 112, 34 );
			this.label11.Name = "label11";
			this.label11.Size = new Size( 61, 17 );
			this.label11.TabIndex = 73;
			this.label11.Text = "Picked:";
			this.label11.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// label13
			// 
			this.label13.AutoSize = true;
			this.label13.BackColor = Color.MediumSlateBlue;
			this.label13.FlatStyle = FlatStyle.Flat;
			this.label13.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.label13.ForeColor = Color.DarkSlateBlue;
			this.label13.Location = new Point( 112, 65 );
			this.label13.Name = "label13";
			this.label13.Size = new Size( 49, 17 );
			this.label13.TabIndex = 70;
			this.label13.Text = "Price:";
			this.label13.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// label14
			// 
			this.label14.AutoSize = true;
			this.label14.BackColor = Color.MediumSlateBlue;
			this.label14.FlatStyle = FlatStyle.Flat;
			this.label14.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.label14.ForeColor = Color.DarkSlateBlue;
			this.label14.Location = new Point( 211, 65 );
			this.label14.Name = "label14";
			this.label14.Size = new Size( 17, 17 );
			this.label14.TabIndex = 60;
			this.label14.Text = "$";
			this.label14.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// Price2Lbl
			// 
			this.Price2Lbl.AutoSize = true;
			this.Price2Lbl.BackColor = Color.MediumSlateBlue;
			this.Price2Lbl.FlatStyle = FlatStyle.Flat;
			this.Price2Lbl.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Price2Lbl.ForeColor = Color.Snow;
			this.Price2Lbl.Location = new Point( 162, 65 );
			this.Price2Lbl.Name = "Price2Lbl";
			this.Price2Lbl.Size = new Size( 48, 17 );
			this.Price2Lbl.TabIndex = 59;
			this.Price2Lbl.Text = "12.30";
			this.Price2Lbl.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// panel10
			// 
			this.panel10.BackColor = Color.DarkSlateBlue;
			this.panel10.Controls.Add( this.Food2Pbx );
			this.panel10.ForeColor = SystemColors.ControlText;
			this.panel10.Location = new Point( 3, 3 );
			this.panel10.Margin = new Padding( 3, 2, 3, 2 );
			this.panel10.Name = "panel10";
			this.panel10.Size = new Size( 100, 80 );
			this.panel10.TabIndex = 53;
			// 
			// Food2Pbx
			// 
			this.Food2Pbx.BackColor = Color.MediumSlateBlue;
			this.Food2Pbx.ForeColor = SystemColors.ControlText;
			this.Food2Pbx.Location = new Point( 3, 2 );
			this.Food2Pbx.Margin = new Padding( 3, 2, 3, 2 );
			this.Food2Pbx.Name = "Food2Pbx";
			this.Food2Pbx.Size = new Size( 94, 76 );
			this.Food2Pbx.SizeMode = PictureBoxSizeMode.StretchImage;
			this.Food2Pbx.TabIndex = 55;
			this.Food2Pbx.TabStop = false;
			// 
			// Food2Name
			// 
			this.Food2Name.AutoSize = true;
			this.Food2Name.BackColor = Color.MediumSlateBlue;
			this.Food2Name.FlatStyle = FlatStyle.Flat;
			this.Food2Name.Font = new Font( "Lucida Bright", 13.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Food2Name.ForeColor = Color.Snow;
			this.Food2Name.Location = new Point( 242, 5 );
			this.Food2Name.Name = "Food2Name";
			this.Food2Name.Size = new Size( 115, 22 );
			this.Food2Name.TabIndex = 54;
			this.Food2Name.Text = "FoodName";
			this.Food2Name.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// panel12
			// 
			this.panel12.BackColor = Color.DarkSlateBlue;
			this.panel12.Controls.Add( this.panel14 );
			this.panel12.ForeColor = SystemColors.ControlText;
			this.panel12.Location = new Point( 3, 2 );
			this.panel12.Margin = new Padding( 3, 2, 3, 2 );
			this.panel12.Name = "panel12";
			this.panel12.Size = new Size( 374, 91 );
			this.panel12.TabIndex = 67;
			// 
			// panel14
			// 
			this.panel14.BackColor = Color.MediumSlateBlue;
			this.panel14.Controls.Add( this.Increase1Btn );
			this.panel14.Controls.Add( this.Decrease1Btn );
			this.panel14.Controls.Add( this.Picked1Lbl );
			this.panel14.Controls.Add( this.label4 );
			this.panel14.Controls.Add( this.Left1Lbl );
			this.panel14.Controls.Add( this.label2 );
			this.panel14.Controls.Add( this.label12 );
			this.panel14.Controls.Add( this.label10 );
			this.panel14.Controls.Add( this.Price1Lbl );
			this.panel14.Controls.Add( this.panel18 );
			this.panel14.Controls.Add( this.Food1Name );
			this.panel14.ForeColor = SystemColors.ControlText;
			this.panel14.Location = new Point( 2, 2 );
			this.panel14.Margin = new Padding( 3, 2, 3, 2 );
			this.panel14.Name = "panel14";
			this.panel14.Size = new Size( 370, 86 );
			this.panel14.TabIndex = 59;
			// 
			// Increase1Btn
			// 
			this.Increase1Btn.BackColor = Color.DarkSlateBlue;
			this.Increase1Btn.Font = new Font( "Lucida Bright", 13.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Increase1Btn.ForeColor = Color.Snow;
			this.Increase1Btn.Location = new Point( 319, 52 );
			this.Increase1Btn.Margin = new Padding( 3, 2, 3, 2 );
			this.Increase1Btn.Name = "Increase1Btn";
			this.Increase1Btn.Padding = new Padding( 3, 0, 0, 0 );
			this.Increase1Btn.Size = new Size( 39, 28 );
			this.Increase1Btn.TabIndex = 81;
			this.Increase1Btn.Text = "+";
			this.Increase1Btn.UseVisualStyleBackColor = false;
			this.Increase1Btn.Click += this.ChangeItemQuantity_Click;
			// 
			// Decrease1Btn
			// 
			this.Decrease1Btn.BackColor = Color.DarkSlateBlue;
			this.Decrease1Btn.Font = new Font( "Lucida Bright", 13.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Decrease1Btn.ForeColor = Color.Snow;
			this.Decrease1Btn.Location = new Point( 246, 52 );
			this.Decrease1Btn.Margin = new Padding( 3, 2, 3, 2 );
			this.Decrease1Btn.Name = "Decrease1Btn";
			this.Decrease1Btn.Padding = new Padding( 3, 0, 0, 0 );
			this.Decrease1Btn.Size = new Size( 39, 28 );
			this.Decrease1Btn.TabIndex = 80;
			this.Decrease1Btn.Text = "-";
			this.Decrease1Btn.UseVisualStyleBackColor = false;
			this.Decrease1Btn.Click += this.ChangeItemQuantity_Click;
			// 
			// Picked1Lbl
			// 
			this.Picked1Lbl.AutoSize = true;
			this.Picked1Lbl.BackColor = Color.MediumSlateBlue;
			this.Picked1Lbl.FlatStyle = FlatStyle.Flat;
			this.Picked1Lbl.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Picked1Lbl.ForeColor = Color.Snow;
			this.Picked1Lbl.Location = new Point( 186, 34 );
			this.Picked1Lbl.Name = "Picked1Lbl";
			this.Picked1Lbl.Size = new Size( 17, 17 );
			this.Picked1Lbl.TabIndex = 79;
			this.Picked1Lbl.Text = "3";
			this.Picked1Lbl.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// label4
			// 
			this.label4.AutoSize = true;
			this.label4.BackColor = Color.MediumSlateBlue;
			this.label4.FlatStyle = FlatStyle.Flat;
			this.label4.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.label4.ForeColor = Color.DarkSlateBlue;
			this.label4.Location = new Point( 112, 5 );
			this.label4.Name = "label4";
			this.label4.Size = new Size( 41, 17 );
			this.label4.TabIndex = 78;
			this.label4.Text = "Left:";
			this.label4.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// Left1Lbl
			// 
			this.Left1Lbl.AutoSize = true;
			this.Left1Lbl.BackColor = Color.MediumSlateBlue;
			this.Left1Lbl.FlatStyle = FlatStyle.Flat;
			this.Left1Lbl.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Left1Lbl.ForeColor = Color.Snow;
			this.Left1Lbl.Location = new Point( 162, 5 );
			this.Left1Lbl.Name = "Left1Lbl";
			this.Left1Lbl.Size = new Size( 17, 17 );
			this.Left1Lbl.TabIndex = 77;
			this.Left1Lbl.Text = "9";
			this.Left1Lbl.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// label2
			// 
			this.label2.AutoSize = true;
			this.label2.BackColor = Color.MediumSlateBlue;
			this.label2.FlatStyle = FlatStyle.Flat;
			this.label2.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.label2.ForeColor = Color.DarkSlateBlue;
			this.label2.Location = new Point( 112, 34 );
			this.label2.Name = "label2";
			this.label2.Size = new Size( 61, 17 );
			this.label2.TabIndex = 73;
			this.label2.Text = "Picked:";
			this.label2.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// label12
			// 
			this.label12.AutoSize = true;
			this.label12.BackColor = Color.MediumSlateBlue;
			this.label12.FlatStyle = FlatStyle.Flat;
			this.label12.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.label12.ForeColor = Color.DarkSlateBlue;
			this.label12.Location = new Point( 112, 65 );
			this.label12.Name = "label12";
			this.label12.Size = new Size( 49, 17 );
			this.label12.TabIndex = 70;
			this.label12.Text = "Price:";
			this.label12.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// label10
			// 
			this.label10.AutoSize = true;
			this.label10.BackColor = Color.MediumSlateBlue;
			this.label10.FlatStyle = FlatStyle.Flat;
			this.label10.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.label10.ForeColor = Color.DarkSlateBlue;
			this.label10.Location = new Point( 211, 65 );
			this.label10.Name = "label10";
			this.label10.Size = new Size( 17, 17 );
			this.label10.TabIndex = 60;
			this.label10.Text = "$";
			this.label10.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// Price1Lbl
			// 
			this.Price1Lbl.AutoSize = true;
			this.Price1Lbl.BackColor = Color.MediumSlateBlue;
			this.Price1Lbl.FlatStyle = FlatStyle.Flat;
			this.Price1Lbl.Font = new Font( "Lucida Bright", 10.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Price1Lbl.ForeColor = Color.Snow;
			this.Price1Lbl.Location = new Point( 162, 65 );
			this.Price1Lbl.Name = "Price1Lbl";
			this.Price1Lbl.Size = new Size( 48, 17 );
			this.Price1Lbl.TabIndex = 59;
			this.Price1Lbl.Text = "12.30";
			this.Price1Lbl.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// panel18
			// 
			this.panel18.BackColor = Color.DarkSlateBlue;
			this.panel18.Controls.Add( this.Food1Pbx );
			this.panel18.ForeColor = SystemColors.ControlText;
			this.panel18.Location = new Point( 3, 3 );
			this.panel18.Margin = new Padding( 3, 2, 3, 2 );
			this.panel18.Name = "panel18";
			this.panel18.Size = new Size( 100, 80 );
			this.panel18.TabIndex = 53;
			// 
			// Food1Pbx
			// 
			this.Food1Pbx.BackColor = Color.MediumSlateBlue;
			this.Food1Pbx.ForeColor = SystemColors.ControlText;
			this.Food1Pbx.Location = new Point( 3, 2 );
			this.Food1Pbx.Margin = new Padding( 3, 2, 3, 2 );
			this.Food1Pbx.Name = "Food1Pbx";
			this.Food1Pbx.Size = new Size( 94, 76 );
			this.Food1Pbx.SizeMode = PictureBoxSizeMode.StretchImage;
			this.Food1Pbx.TabIndex = 55;
			this.Food1Pbx.TabStop = false;
			// 
			// Food1Name
			// 
			this.Food1Name.AutoSize = true;
			this.Food1Name.BackColor = Color.MediumSlateBlue;
			this.Food1Name.FlatStyle = FlatStyle.Flat;
			this.Food1Name.Font = new Font( "Lucida Bright", 13.8F, FontStyle.Bold, GraphicsUnit.Point,   0 );
			this.Food1Name.ForeColor = Color.Snow;
			this.Food1Name.Location = new Point( 242, 5 );
			this.Food1Name.Name = "Food1Name";
			this.Food1Name.Size = new Size( 115, 22 );
			this.Food1Name.TabIndex = 54;
			this.Food1Name.Text = "FoodName";
			this.Food1Name.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// CoffeeHall
			// 
			this.AutoScaleDimensions = new SizeF( 7F, 15F );
			this.AutoScaleMode = AutoScaleMode.Font;
			this.BackColor = Color.DarkSlateBlue;
			this.ClientSize = new Size( 509, 394 );
			this.Controls.Add( this.panel1 );
			this.FormBorderStyle = FormBorderStyle.FixedToolWindow;
			this.Margin = new Padding( 3 );
			this.Name = "CoffeeHall";
			this.StartPosition = FormStartPosition.CenterScreen;
			this.FormClosed += this.CoffeeHall_FormClosed;
			this.panel1.ResumeLayout( false );
			this.panel17.ResumeLayout( false );
			this.panel21.ResumeLayout( false );
			this.panel21.PerformLayout();
			this.panel2.ResumeLayout( false );
			this.panel3.ResumeLayout( false );
			this.panel16.ResumeLayout( false );
			this.panel15.ResumeLayout( false );
			this.panel15.PerformLayout();
			this.panel20.ResumeLayout( false );
			this.panel19.ResumeLayout( false );
			this.panel19.PerformLayout();
			this.panel4.ResumeLayout( false );
			this.panel5.ResumeLayout( false );
			this.panel11.ResumeLayout( false );
			this.panel13.ResumeLayout( false );
			this.panel13.PerformLayout();
			this.panel24.ResumeLayout( false );
			(( System.ComponentModel.ISupportInitialize )this.Food3Pbx).EndInit();
			this.panel7.ResumeLayout( false );
			this.panel8.ResumeLayout( false );
			this.panel8.PerformLayout();
			this.panel10.ResumeLayout( false );
			(( System.ComponentModel.ISupportInitialize )this.Food2Pbx).EndInit();
			this.panel12.ResumeLayout( false );
			this.panel14.ResumeLayout( false );
			this.panel14.PerformLayout();
			this.panel18.ResumeLayout( false );
			(( System.ComponentModel.ISupportInitialize )this.Food1Pbx).EndInit();
			this.ResumeLayout( false );
		}

		#endregion

		private Panel panel1;
		private Panel panel2;
		private Panel panel3;
		private Panel panel17;
		private Panel panel21;
		private Label label1;
		private Panel panel4;
		private Panel panel5;
		private Panel panel12;
		private Panel panel14;
		private Label label2;
		private Label label12;
		private Label label10;
		private Label Price1Lbl;
		private Panel panel18;
		private PictureBox Food1Pbx;
		private Label Food1Name;
		private Panel panel20;
		private Panel panel19;
		private Label label20;
		private Label TotalPrice;
		private Label label21;
		private Panel panel16;
		private Panel panel15;
		private Panel panel6;
		private RadioButton DrinkRbtn;
		private Label label27;
		private RadioButton FoodRbtn;
		private Label label26;
		private Button BuyBtn;
		private Panel panel25;
		private Label Picked1Lbl;
		private Label label4;
		private Label Left1Lbl;
		private Button Increase1Btn;
		private Button Decrease1Btn;
		private Panel panel11;
		private Panel panel13;
		private Button Increase3Btn;
		private Button Decrease3Btn;
		private Label Picked3Lbl;
		private Label label18;
		private Label Left3Lbl;
		private Label label24;
		private Label label25;
		private Label label28;
		private Label Price3Lbl;
		private Panel panel24;
		private PictureBox Food3Pbx;
		private Label Food3Name;
		private Panel panel7;
		private Panel panel8;
		private Button Increase2Btn;
		private Button Decrease2Btn;
		private Label Picked2Lbl;
		private Label label6;
		private Label Left2Lbl;
		private Label label11;
		private Label label13;
		private Label label14;
		private Label Price2Lbl;
		private Panel panel10;
		private PictureBox Food2Pbx;
		private Label Food2Name;
		private LinkLabel helpLbl;
	}
}